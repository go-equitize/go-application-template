...
func (r *Request[R, E]) Execute(ctx context.Context, method string, url string) (*R, *E, error) {
	r.SetContext(ctx)
	...
	if res.StatusCode() >= http.StatusBadRequest {
		if r.LogReqRes {
			logger.Infof(ctx, "[HTTP Client] Send %s request to %s error, request params: %s, request body: %s, response status: %s, response body: %s", method, url, r.QueryParam.Encode(), reqBodyJson, res.Status(), res.Body())
		} else {
			logger.Infof(ctx, "[HTTP Client] Send %s request to %s error, status: %s. ", method, url, res.Status())
		}

		var errRes E
		if err := json.Unmarshal(res.Body(), &errRes); err != nil {
			if r.LogReqRes {
				logger.Warnf(ctx, "[HTTP Client] Can't unmarshal err response, error response: %s", res.Body())
			} else {
				logger.Warnf(ctx, "[HTTP Client] Can't unmarshal err response")
			}
			return nil, nil, err
		}
	...
	if err := json.Unmarshal(res.Body(), &resData); err != nil {
		if r.LogReqRes {
			logger.Warnf(ctx, "[HTTP Client] Can't unmarshal response body, res: %s", res.Body())
		} else {
			logger.Warnf(ctx, "[HTTP Client] Can't unmarshal response body")
		}
		return nil, nil, err
	}

	if r.LogReqRes {
		logger.Infof(ctx, "[HTTP Client] Send %s request to %s successfully, request params: %s, request body: %s, response status: %s, response body: %s", method, url, r.QueryParam.Encode(), reqBodyJson, res.Status(), res.Body())
	} else {
		logger.Infof(ctx, "[HTTP Client] Send %s request to %s successfully", method, url)
	}

	return &resData, nil, nil
}

func (r *Request[R, E]) Get(ctx context.Context, url string) (*R, *E, error) {
	return r.Execute(ctx, http.MethodGet, url)
}

func (r *Request[R, E]) Post(ctx context.Context, url string) (*R, *E, error) {
	return r.Execute(ctx, http.MethodPost, url)
}

func (r *Request[R, E]) Put(ctx context.Context, url string) (*R, *E, error) {
	return r.Execute(ctx, http.MethodPut, url)
}

func (r *Request[R, E]) Patch(ctx context.Context, url string) (*R, *E, error) {
	return r.Execute(ctx, http.MethodPatch, url)
}

func (r *Request[R, E]) Delete(ctx context.Context, url string) (*R, *E, error) {
